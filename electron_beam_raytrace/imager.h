...
namespace Imager
{
    ...
    struct Energy
    {
      ...
      double green;
      double blue;
    Energy(double _red, double _green, double _blue, double _energy = 100)
    : red  (_energy * _red)
    , green(_energy * _green)
    , blue (_energy * _blue)
      {
      }
    ...
    inline Energy operator * (const Energy& aEnergy, const Energy& bEnergy)
    {
        return Energy(
            aEnergy.red   * bEnergy.red,
            aEnergy.green * bEnergy.green,
            aEnergy.blue  * bEnergy.blue);
    }

    inline Energy operator * (double scalar, const Energy &energy)
    {
        return Energy(
            scalar * energy.red, 
            scalar * energy.green, 
    ...
    inline Energy operator + (const Energy& a, const Energy& b)
    {
        return Energy(
            a.red   + b.red,
            a.green + b.green,
    ...
    private:
        ...
        const double K1;    // distance from center of tube to outside of tube
    };
    ...
    //------------------------------------------------------------------------
    struct ElectronSource: public Taggable
    {
      ElectronSource(const Vector& _location, const Color& _color, std::string _tag = "")
	: Taggable(_tag)
	, location(_location)
	,color(_color)
      {
